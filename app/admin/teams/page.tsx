...
export default function AdminTeams() {
  ...
  const createFullTeam = async () => {
    if (!name.trim()) {
      notify.error('Team name is required.')
      return
    }

    if (!presidentName.trim() || !vpName.trim() || !secretaryName.trim() || !finSecName.trim()) {
      notify.error('All candidate names are required for full team creation.')
      return
    }
    ...
    try {
      ...
      formData.append('president_name', presidentName.trim())
      formData.append('vp_name', vpName.trim())
      formData.append('secretary_name', secretaryName.trim())
      formData.append('financial_secretary_name', finSecName.trim())

      if (presImage) {
        formData.append('president_image', presImage)
      } else if (presImagePath) {
        formData.append('president_image_url', presImagePath)
      }

      if (vpImage) {
        formData.append('vp_image', vpImage)
      } else if (vpImagePath) {
        formData.append('vp_image_url', vpImagePath)
      }

      if (secImage) {
        formData.append('secretary_image', secImage)
      } else if (secImagePath) {
        formData.append('secretary_image_url', secImagePath)
      }

      if (finImage) {
        formData.append('financial_secretary_image', finImage)
      } else if (finImagePath) {
        formData.append('financial_secretary_image_url', finImagePath)
      }

      await api.post('/api/teams/full', formData)

      notify.success('Full team roster created successfully.')
      resetForm()
      fetchTeams()
    } catch (err: any) {
      notify.error(err?.response?.data?.detail || err?.response?.data || 'Failed to create full team roster.')
    }
  }

  const createTeam = async () => {
    if (advanced) {
      await createFullTeam()
      return
    }
      ...
      <section className="space-y-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-300">Once created, full roster members are stored in the candidates table and count toward the team candidate total.</p>
            <p className="mt-2 text-sm text-gold underline hover:text-white"><a href="/admin/candidates">View saved candidate table</a></p>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-[1fr_260px] md:items-end">
            <div className="space-y-4">
              ...
              <button
                type="button"
                onClick={createTeam}
                className="inline-flex h-full w-full items-center justify-center rounded-2xl bg-gold px-5 py-3 text-sm font-semibold text-navy transition hover:bg-[#b79431]"
              >
                {advanced ? 'Add full team roster' : 'Add team'}
              </button>
            </div>
          </div>
        </div>
                    ...
                    <tr key={team.id} className="hover:bg-white/5">
                      <td className="px-3 py-4">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span className="rounded-full bg-gold/15 px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] text-gold">
                            {team.candidate_count ?? 0} candidate{team.candidate_count === 1 ? '' : 's'}
                          </span>
                        </div>
